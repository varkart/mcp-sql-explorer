...
describe('MariaDB Adapter Integration', function () {
  this.timeout(300000);
  ...
  before(async () => {
    ...
    const mariadbConfig = containers.getConfig('mariadb');
    if (!mariadbConfig) {
      throw new Error('MariaDB container not available');
    }
  ...
  describe('Connection', () => {
    ...
    it('should disconnect from MariaDB', async () => {
      await adapter.disconnect();
  ...
  describe('Query Execution', () => {
    ...
    it('should insert and select data', async () => {
      ...
      await adapter.execute(
        'INSERT INTO test_items (category_id, name) VALUES (?, ?), (?, ?)',
        [1, 'Novel', 2, 'Chess']
      );

      const result = await adapter.execute('SELECT * FROM test_items ORDER BY id');
  ...
  describe('Schema Introspection', () => {
    ...
    it('should retrieve schema information', async () => {
      ...
      expect(schema.databaseType).to.equal('mariadb');
  ...
  describe('Read-Only Mode', () => {
    it('should reject writes at the database in read-only mode', async () => {
      if (!adapter.isConnected()) {
        await adapter.connect(config);
      }
      ...
      try {
        await adapter.execute('INSERT INTO test_categories (name) VALUES (?)', ['Toys']);
        expect.fail('Should have thrown an error in read-only mode');
      } catch (error) {
        expect((error as Error).message).to.match(/read only/i);
      }

      const result = await adapter.execute('SELECT * FROM test_categories');
      expect(result.rows).to.have.lengthOf(2);
  ...
  describe('Schema Intelligence', () => {
    ...
    it('should execute a sample query', async () => {
      const sql = adapter.buildSampleQuery('test_items', undefined, 1);
    ...
    it('should explain a query', async () => {
      const result = await adapter.explain('SELECT * FROM test_items WHERE id = 1');
